...
Same API as ``board/board.py`` in the project root. Workers import this module from
their own folder after ``launch`` sets ``cwd`` to the worker directory and
``BOARD_PATH`` to ``site/board.sqlite``, so they share one board with the orchestrator
...
def reset_board(path: Path = BOARD_PATH) -> None:
    """Drop and recreate ``todos`` (standalone demo only — arcade mode uses orchestrator reset)."""
...
def add_goal(task: str, path: Path = BOARD_PATH) -> int:
    """Insert a top-level goal; return its id."""
    with _connect(path) as conn:
...
def add_step(goal_id: int, task: str, path: Path = BOARD_PATH) -> int:
    """Insert a child step under a goal."""
    with _connect(path) as conn:
        cur = conn.execute(
...
def list_todos(path: Path = BOARD_PATH) -> list[dict]:
    """Return every todo row (standalone mode)."""
    with _connect(path) as conn:
        rows = conn.execute(
...
def list_goal_and_steps(goal_id: int, path: Path = BOARD_PATH) -> list[dict]:
    """Scoped view: one goal and only its steps (arcade / parallel workers)."""
    with _connect(path) as conn:
        rows = conn.execute(
...
def complete_todo(task_id: int, result: str, path: Path = BOARD_PATH) -> None:
    """Mark a todo ``done`` and store a short result."""
    with _connect(path) as conn:
        conn.execute(
